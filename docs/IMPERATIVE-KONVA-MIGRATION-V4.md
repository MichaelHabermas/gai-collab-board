# Imperative Konva Migration Plan (V4)

**Supersedes:** IMPERATIVE-KONVA-MIGRATION-V3. Key rules: merge to `development`; E2E path `tests/e2e/`; Zustand vanilla `subscribe(listener)` receives full store state; drag split into five modules (DragCoordinator, dragCommit, alignmentEngine, dragBounds, frameDragReparenting); explicit S-plan dependency contract (§4).
...
| CanvasHost/useCanvasSetup LOC creep | 🟡 Medium | Medium | Plan mandates 300-line cap on both files. Reviewer checklist. |
| Drag sub-module grows past 200 LOC | 🟡 Medium | Medium | Plan mandates 200-line cap per drag module. Reviewer checklist. |
...
| **All merge targets = development** | Git workflow requires development branch. | Merge to main (rejected: violates project rules) |
| **Explicit STATE-MANAGEMENT-PLAN-2 dependency contract** | S1/S2 merged. No hard dep on S3-S7. Retest after store API changes. | No ordering (rejected: two workstreams silently break each other) |
...
Automated Playwright mobile emulation covers pinch-zoom and touch drag.
...
*End of IMPERATIVE-KONVA-MIGRATION-V4.md*
