# Imperative Konva Migration Plan V3 — The Corrected One

> **V2 post-mortem:** V2 had the right architecture and honest LOC estimates, but the reviewer found five critical bugs that would've caused rework or failure. This plan fixes all five, preserves every sound V2 decision, and adds the missing dependency contract with STATE-MANAGEMENT-PLAN-2.

## V2 Bugs Fixed

| # | V2 Bug | V3 Fix | Section |
|---|--------|--------|---------|
| 1 | Zustand subscription API wrong — claimed `subscribe()` uses selector output | Zustand v5 vanilla `subscribe(listener: (state: T, prevState: T) => void)` — provides full store state. Manager checks `state.objects !== prevState.objects`. | Appendix C |
| 2 | E2E paths wrong — used `e2e/` | Correct path: `tests/e2e/`. All references fixed. | Epic 0 §0.3 |
| 3 | "Merge to main" violates git workflow | All DoDs say `development` (or current spike branch). | Every Epic DoD |
| 4 | DragEngine 600 LOC monolith — recreates the problem V2 criticizes | Split into 5 modules: `DragCoordinator` (50), `dragCommit` (200), `alignmentEngine` (150), `dragBounds` (80), `frameDragReparenting` (120). | Epic 3 |
| 5 | No ordering with STATE-MANAGEMENT-PLAN-2 | Explicit dependency contract: S1/S2 merged (prereq met), no hard dep on S3-S7, retest after store API changes. | §4 |

**Additional V3 improvements:**

- CanvasHost split: `CanvasHost.tsx` (~250 LOC) + `useCanvasSetup.ts` (~200 LOC) to stay under 300-line limit
- Corrected deletion manifest: 26 dying files totaling ~4,907 LOC (V2 missed 7 hooks, undercounted by 346)
- Mobile automated testing: Playwright mobile emulation specs replace "manual test on mobile"
- Baseline tooling specified: concrete script path and implementation contract
...
17. [Appendix B: Dying Code Manifest (Corrected)](#appendix-b-dying-code-manifest-corrected)
18. [Appendix C: Zustand v5 Subscription Contract](#appendix-c-zustand-v5-subscription-contract)
19. [Appendix D: Playwright Mobile Emulation Specs](#appendix-d-playwright-mobile-emulation-specs)
...
**What V1 got right:** The architectural direction.

**What V1 got wrong:** LOC estimates (claimed 150-line CanvasHost), extraction framing (called a 761-line rewrite an "extraction"), ignored bitmap caching, ignored per-shape subscription model, no E2E task list, no performance baselines.

**What V2 got right:** Honest LOC, constitutional amendments, O(changed) diff design, connector deduplication, bitmap caching strategy, separate cutover/cleanup PRs, risk matrix.

**What V2 got wrong:** Five critical bugs documented above. This plan fixes all five.
...
### Subscription Flow (Corrected — V3 Bug Fix #1)
...
See **Appendix B** for the corrected, complete manifest with LOC counts.

**Summary:** 26 dying files totaling ~4,907 LOC (V2 counted 4,561 — missed 7 hooks worth 346 LOC).
...
> **V3 Bug Fix #5.** V2 had no ordering with STATE-MANAGEMENT-PLAN-2. This section establishes the contract.
...
src/canvas/                              # NEW top-level module
  ...
  drag/                                  # V3 FIX: DragEngine decomposed into sub-modules
...
**Every file stays under the 300-line project limit.** The V2 DragEngine (600 LOC) and CanvasHost (400-500 LOC) both violated this. V3 fixes both.
...
Add the following to `docs/CONSTITUTION.md` as Articles XX–XXVII (additive to Articles I–XIX).
...
#### Article XXII — Subscription Efficiency (CORRECTED from V2)
...
2. Stateless logic must remain as pure functions: drag sub-modules (`dragCommit`, `alignmentEngine`, `dragBounds`, `frameDragReparenting`) export functions, `ShapeEventWiring` exports a `wireEvents()` function, shape factories export `create()`/`update()` functions.
3. Controllers with simple state machines (`DrawingController`, `MarqueeController`, `ConnectorController`, `DragCoordinator`) use closure-based modules (factory functions returning method objects). Classes are acceptable only if the state management genuinely benefits from `this` context — justify in PR description.
...
**Baseline tooling contract (V3 improvement):**

Create `scripts/capture-perf-baseline.ts` with:

- Programmatic Playwright test that creates a 1000-object board, measures TTI
- Zustand selector counter instrumentation (monkey-patch selectors, count calls during a timed drag)
- Output format: JSON matching `docs/perf-baselines/pre-migration.json` schema
- Must be runnable via `bun run scripts/capture-perf-baseline.ts`
- Estimated LOC: ~150
...
**All paths use `tests/e2e/` (V3 Bug Fix #2).**
...
| Mobile touch (pinch-zoom) | `tests/e2e/mobilePinchZoom.spec.ts` | Playwright mobile emulation → pinch gesture → zoom changes |
| Mobile touch (drag) | `tests/e2e/mobileDrag.spec.ts` | Playwright mobile emulation → touch drag shape → position updates |
...
**17 new tests** (V2 said 12 — undercounted. V3 adds 2 mobile tests and corrects several V2 missed scenarios).

See **Appendix D** for mobile emulation test specs.

### 6.4 — Epic 0 Definition of Done

- [ ] Constitutional amendments (Articles XX–XXVII) added to `docs/CONSTITUTION.md`
- [ ] `scripts/capture-perf-baseline.ts` written and producing valid JSON
- [ ] Performance baselines captured and saved to `docs/perf-baselines/pre-migration.json`
- [ ] All 17 new E2E tests written and passing against current codebase
- [ ] All existing E2E tests still pass
- [ ] `bun run validate` passes
- [ ] **PR merged to `development`** (V3 Bug Fix #3 — NOT main)
...
```typescript
// ~80 LOC — Closure-based module (Article XXVI.3)
...
### DragCoordinator.ts (~50 LOC) — V3 Bug Fix #4
...
- [ ] **Every drag sub-module stays under 200 LOC** (V3 Bug Fix #4)
...
### Why Two Files (V3 Improvement)

V2 had a single 400-500 LOC CanvasHost.tsx that would violate the 300-line project limit. V3 splits it:
...
| Touch (pinch-to-zoom, drag) | Mobile emulation E2E (Appendix D) |
...
5. **Run full E2E suite** — All 17+ Epic 0 tests must pass + all existing tests.
6. **Run mobile E2E tests** — Playwright mobile emulation (Appendix D).
7. **Run performance comparison** — Capture post-migration baselines.
8. **Manual test matrix** — Every row in integration checklist verified.

### Epic 5 Definition of Done

- [ ] `<CanvasHost>` renders and manages all shape types
- [ ] **CanvasHost.tsx ≤ 300 LOC, useCanvasSetup.ts ≤ 300 LOC** (V3 improvement)
- [ ] All E2E tests pass (17 new from Epic 0 + all existing)
- [ ] Mobile E2E tests pass (Appendix D)
- [ ] All items in integration checklist verified
- [ ] Performance baselines captured (post-migration)
- [ ] `bun run validate` passes
- [ ] **PR merged to `development`** (V3 Bug Fix #3)
...
### Deletion Manifest (Corrected — V3 Improvement)
...
|------|----------|-----------|-----------|
| Feature regression on cutover | 🔴 Critical | Medium | 17 E2E tests from Epic 0 + manual test matrix in Epic 5 |
...
| CanvasHost/useCanvasSetup LOC creep | 🟡 Medium | Medium | V3 mandates 300-line cap on both files. Reviewer checklist. |
| Drag sub-module grows past 200 LOC | 🟡 Medium | Medium | V3 mandates 200-line cap per drag module. Reviewer checklist. |
| Text overlay positioning broken | 🟡 Medium | Low | canvasTextEditOverlay.ts verified against Konva.Node. E2E tests. |
| Z-order mismatch | 🟡 Medium | Medium | Frames `moveToBottom()` on static layer. Unit test. |
| Memory leaks (undestroyed nodes) | 🟡 Medium | Medium | `removeNode` calls `node.destroy()`. Dev-mode leak detection. |
| Touch/mobile regression | 🟡 Medium | Medium | **Playwright mobile emulation E2E (Appendix D)** — automated, not manual. |
| Bundle doesn't shrink | 🟢 Low | Low | react-konva ~45KB gzipped. Removal is guaranteed improvement. |
| Frame child count not updating | 🟡 Medium | Low | KonvaNodeManager reads `frameChildrenIndex`. Unit test. |
...
| **Split DragEngine into 5 modules (V3)** | V2's 600 LOC DragEngine recreated the monolith. Existing seams (alignmentGuides.ts, snapToGrid.ts, useFrameContainment.ts) prove the algorithms are separable. Each module ≤200 LOC. | Single DragEngine.ts (rejected: 600 LOC violates 300-line limit, hard to test) |
| **Split CanvasHost into CanvasHost + useCanvasSetup (V3)** | V2's 400-500 LOC CanvasHost violates 300-line limit. Manager instantiation + subscription wiring is a cohesive unit that extracts cleanly. | Single CanvasHost.tsx (rejected: 500+ LOC, hard to reason about) |
| Classes for node lifecycle, functions for stateless logic | Konva nodes are inherently mutable with lifecycle. Classes justified per Article XXVI. | All classes (unnecessary OOP for pure functions) |
...
| **All merge targets = development (V3)** | Git workflow requires development branch. V2 said "main" — wrong. | Merge to main (rejected: violates project rules) |
| **Explicit STATE-MANAGEMENT-PLAN-2 dependency contract (V3)** | S1/S2 merged. No hard dep on S3-S7. Retest after store API changes. | No ordering (rejected: two workstreams silently break each other) |
...
## Appendix B: Dying Code Manifest (Corrected)

V2 counted 4,561 LOC across 19 files. V3 corrects this to ~4,907 LOC across 26 files (V2 missed 7 hooks).
...
### Hooks (10 files, ~1,533 LOC) — V3 corrects V2 omissions
...
| **Grand total dying code** | **~4,907** (V2: 4,561 — undercounted by 346) |

---

## Appendix C: Zustand v5 Subscription Contract

> **V3 Bug Fix #1.** V2 stated `useObjectsStore.subscribe()` provides `(nextState, prevState)` via selector-based subscription. This was wrong.
...
V2 Article XXII stated:
> `useObjectsStore.subscribe()` provides `(nextState, prevState)`.

This is technically correct for Zustand v5 vanilla — but V2's usage context implied selector-style subscription where the callback receives projected values. The V2 code examples showed `handleStoreChange(nextObjects, prevObjects)` being called directly from the subscriber, suggesting the subscriber would receive just the objects. In reality, it receives the full state, and the caller must extract `.objects` from both.

V2's CanvasHost code (line 1072-1075) also showed:
...
This 2-argument `subscribe(selector, listener)` pattern is `subscribeWithSelector` middleware — **not available** in our stores. The correct pattern is the 1-argument `subscribe(listener)` shown above.

---

## Appendix D: Playwright Mobile Emulation Specs

> **V3 improvement.** V2 said "manual test on mobile" for touch/mobile regression. This is weak for a plan that claims to be bulletproof. V3 adds automated Playwright mobile emulation tests.

### `tests/e2e/mobilePinchZoom.spec.ts`
...
test('pinch-to-zoom changes viewport scale', async ({ page }) => {
  ...
  // Simulate pinch gesture using Playwright's touchscreen API
  ...
  // Record initial viewport scale
  const initialScale = await page.evaluate(() =>
    document.querySelector('[data-testid="zoom-display"]')?.textContent
  );

  // Pinch out (zoom in) using two-finger gesture
  await page.touchscreen.pinch(centerX, centerY, 50, 150);
...
test('touch drag moves shape to new position', async ({ page }) => {
  await page.goto('/board/test-board-id');
  await page.waitForSelector('[data-testid="canvas-container"]');
  ...
  // Touch drag the shape
  const shape = page.locator('[data-testid="shape-test-id"]');
  const shapeBounds = await shape.boundingBox();
  if (!shapeBounds) throw new Error('Shape not found');

  const startX = shapeBounds.x + shapeBounds.width / 2;
  const startY = shapeBounds.y + shapeBounds.height / 2;
  ...
  // ... (check Firestore or store state)
});
```

**Note:** Playwright's mobile emulation handles viewport sizing, touch events, and device pixel ratio. These tests complement the desktop E2E tests and catch touch-specific regressions that manual testing might miss.

---

*End of IMPERATIVE-KONVA-MIGRATION-V3.md*
